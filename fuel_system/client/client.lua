...
-- LegacyFuel compatibility helper (يتعامل مع الصادرات أو يسقط على النيتف)
local function _callFuelExport(resourceName, fnName, ...)
    if not exports or not resourceName or resourceName == '' then return false end

    local resource = exports[resourceName]
    if not resource then return false end

    local fn = resource[fnName]
...
local function _tryFuelExports(name, ...)
    local current = GetCurrentResourceName()
    if current and current ~= '' then
        local ok, result = _callFuelExport(current, name, ...)
        if ok then return ok, result end
    end

    local ok, result = _callFuelExport('LegacyFuel', name, ...)
    if ok then return ok, result end
...
local function GetVehicleFuelPercent(veh)
    ...
    local ok, result = _tryFuelExports('GetFuel', veh)
...
local function SetVehicleFuelPercent(veh, level)
    if not veh or veh == 0 or not DoesEntityExist(veh) then return 0 end

    local fuel = tonumber(level) or 0.0
    if fuel < 0 then fuel = 0 elseif fuel > 100 then fuel = 100 end

    local ok = _tryFuelExports('SetFuel', veh, fuel)
    if not ok then
        SetVehicleFuelLevel(veh, fuel + 0.0)
        if Config.SyncFuelBetweenPlayers then
            Entity(veh).state:set('qb-fuel', fuel + 0.0, true)
        end
    elseif Config.SyncFuelBetweenPlayers then
        -- التصدير الخاص بالمورد يحدّث الحالة بالفعل، لكننا نعيد الضبط هنا لضمان التوافق مع أي مورد خارجي
        Entity(veh).state:set('qb-fuel', fuel + 0.0, true)
    end
...
local refuelVehicle = function (veh)
    ...
    local vehFuel = math.floor(GetVehicleFuelPercent(veh) or 0)
...
local refillVehicleFuel = function (liter)
    ...
    SimpleProgress('fueling_vehicle', Lang:t('progress.refueling'), Config.RefillTimePerLitre * liter * 1000, false, true, {
        ...
        -- استخدم الدالة المساعدة (LegacyFuel أو النيتف)
        local currentFuel = math.floor(GetVehicleFuelPercent(veh) or 0)
    ...
    SetVehicleFuelPercent(veh, newFuel)
...
showFuelMenu = function ()
    ...
    SendNUIMessage({
        ...
        currentFuel = math.floor(GetVehicleFuelPercent(veh) or 0),
    })
...
CreateThread(function ()
    while true do
        if __fuelMenuOpen and IsControlJustReleased(0, 322) then -- ESC
            hideFuelMenu()
        end
        Wait(0)
    end
...
RegisterNetEvent('fuel:client:openUi', function()
    showFuelMenu()
end)
