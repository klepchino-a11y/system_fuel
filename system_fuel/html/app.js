...
const $post = async (url, data) => {
    ...
    // ✅ استخدام اسم المورد الديناميكي حسب توثيق Cfx.re (NUI callbacks)
    const resourceName = (typeof GetParentResourceName === 'function') ? GetParentResourceName() : 'fuel_system';
    const result = await fetch(`https://${resourceName}${url}`, {
        method: "POST",
        headers: {
...
const updateLimits = () => {
    ...
    $capacity.innerText = CURRENT_FUEL.toString();
    $liter.max = maxLiter;
...
const setupTranslations = (language) => {
    ...
    const translations = TRANSLATIONS[language] ?? TRANSLATIONS["ar"];
...
$form.addEventListener("submit", (e) => {
    ...
    // ✅ النداء الصحيح للكلباك في الكلاينت
    $post("/refill", { liter });
});
