...
-- ========== إضافة: إعدادات الوقود لكل مركبة + HUD ==========

-- HUD (يمكن تعطيله إن رغبت)
Config.FuelHUD = {
    enabled = true,
    position = { x = 0.90, y = 0.85 }, -- إحداثيات الشاشة (0-1)
    width = 0.12,
    height = 0.02,
}

-- القيم الافتراضية في حال لم يتم تعريف المركبة في القائمة
Config.DefaultVehicleFuel = {
    capacity = 100,   -- حجم التانكي (لتر)
    moving   = 1.50,  -- استهلاك/ثانية أثناء السير (لتر/ث)
    idle     = 0.50,  -- استهلاك/ثانية أثناء الوقوف (لتر/ث)
}

-- الصيغة المطلوبة: model [capacity] [moving] [idle]
-- أمثلة:
-- "sultan [100] [1.50] [0.50]",
-- "adder  [85]  [1.80] [0.60]",
Config.VehicleFuelLines = {
    "sultan [100] [1.50] [0.50]",
    "police [85] [1.80] [0.60]",
    "bus    [200] [2.40] [0.80]",
}
